...
app.get('/api/live-accidents/:year', async (req, res) => {
    ...
    try {
        ...
        // 🚀 核心優化 4：【後端包容性地理圍欄防護網】
        // 故意將網子向外擴大，精準接住政府那些因歷史坐標系偏誤而掉進海裡的離島真實數據！
        const cleanRows = formattedRows.filter(item => {
            // 1. 台灣本島精準陸地範圍 (21.9~25.3, 120.1~122.1)
            if (item.lat >= 21.9 && item.lat <= 25.3 && item.lng >= 120.1 && item.lng <= 122.1) {
                return true;
            }
            
            // 2. 離島包容性範圍（擴大格框，確保偏移的數據不被誤刪）
            // 澎湖特區
            if (item.lat >= 23.1 && item.lat <= 23.8 && item.lng >= 119.2 && item.lng <= 119.8) return true;
            // 金門特區
            if (item.lat >= 24.3 && item.lat <= 24.6 && item.lng >= 118.1 && item.lng <= 118.6) return true;
            // 馬祖特區
            if (item.lat >= 26.0 && item.lat <= 26.4 && item.lng >= 119.8 && item.lng <= 120.2) return true;
            // 綠島特區
            if (item.lat >= 22.5 && item.lat <= 22.8 && item.lng >= 121.3 && item.lng <= 121.6) return true;
            // 蘭嶼特區
            if (item.lat >= 21.9 && item.lat <= 22.2 && item.lng >= 121.4 && item.lng <= 121.7) return true;

            // 完全不符合以上安全邊界的無效髒點（如北極圈或0點點位），在此徹底攔截
            return false;
        });

        console.log(`✨ 終極校正成功！共 ${cleanRows.length} 筆資料完美回歸台灣與離島範圍！`);
...
app.use((req, res) => {
    res.status(404).send('找不到此 API 路徑');
});
